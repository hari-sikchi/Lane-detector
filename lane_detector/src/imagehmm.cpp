...
void hybridmedianfilter(element* image, element* result, int N, int M)
{
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
	...
	if( abs(left_lines[i][1]-90*CV_PI/180)<=hori_margin || abs( 270*CV_PI/180-left_lines[i][1]<=hori_margin)) {left_lines.erase(left_lines.begin()+i); cout<<"deleted"<<endl;checkera[0]=1;}
...
quad_pts.push_back(cv::Point2f(37, 283));
	...
	else if(checkera[1]==1)
		fangle=left_s;
	else
		fangle=(right_s+left_s)/2.0;
